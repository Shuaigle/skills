...
If `issues/` already holds files from a previous run: a file directly under `issues/` whose first line is exactly `<!-- generated by to-tickets -->` is yours — delete the stale ones before writing the approved set, since a stale generated leftover reads as valid frontier work and completed work lives in the code and its commits. Everything else is not yours: leave it in place and tell the user. An unmarked file sitting at a path you were about to write → stop and report; never overwrite it.
