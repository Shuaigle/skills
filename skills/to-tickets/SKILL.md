...
If `issues/` already holds files from a previous run, delete only the ones carrying the `<!-- generated by to-tickets -->` marker before writing the approved set — a stale generated leftover reads as valid frontier work, and completed work lives in the code and its commits. Files without the marker are not yours: leave them in place and tell the user they're there.
